...
void keyExpansion(unsigned char* inputKey, unsigned char* expandedKeys) {
    ...
    for(int i=0; i<16;i++){
        ...
        int bytesGenerated = 16; // till now we have generated 16 bytes
        int rconIteration = 1;  // rcon iteration begins with i
...
void AES_Encrypt(unsigned char* message, unsigned char* key){
    ...
    for(int i=0;i<16;i++)
        state[i] = message[i];
    int numberofRounds = 9; // total round are 10 but the iteration go for 9 because last round contains 3 steps.
    ...
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, expandedKey+160); // the final key begins at expandedKey+160
