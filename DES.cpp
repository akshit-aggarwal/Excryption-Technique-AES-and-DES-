...
class DES{
public:
    int keyi[16][48], total[64], left[32], right[32], ck[28], dk[28], expansion[48], z[48], xor1[48], sub[32], p[32], xor2[32],
    temp[64], pc1[56], ip[64], inv[8][8];
    char final[1000];
    void IP();
    void PermChoice1();
    void Expansion();
...
void DES::xor_1E(int round){
    int i;
    for(i=0;i<48;i++){
        xor1[i]=expansion[i]^keyi[round-1][i];
    }
}
void DES::xor_1D(int round){
    int i;
    for(i=0;i<48;i++){
